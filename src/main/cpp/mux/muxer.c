...
int imp(const char *in, const char *out)
{
    ...
    ret = av_seek_frame(ifmt_ctx, -1, 1*AV_TIME_BASE, AVSEEK_FLAG_ANY);
    if (ret < 0) {
    ...
    while (1) {
        ...
        if (av_q2d(in_stream->time_base) * pkt.pts > 8) {
            av_packet_unref(&pkt);
            break;
        }
        /* copy packet */
...
Java_cn_com_lasong_media_Muxer_remux(JNIEnv
* env,
